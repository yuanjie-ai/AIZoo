...
class Optimizer(object):
    ...
    def run(self, trials=3, direction='maximize', n_jobs=1, plot=True, gc_after_trial=False, seed=777):
        show_progress_bar = False if n_jobs > 1 else True

        self.study = optuna.create_study(direction=direction, sampler=TPESampler(seed=seed))
        self.study.optimize(
            self._objective,
            n_trials=trials,
